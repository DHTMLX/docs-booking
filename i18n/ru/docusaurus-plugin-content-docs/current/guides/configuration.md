...
title: Конфигурация
description: Вы можете узнать о конфигурации в документации библиотеки DHTMLX JavaScript Booking. Изучайте руководства разработчика и справочник API, пробуйте примеры кода и живые демо, а также скачайте бесплатную 30-дневную ознакомительную версию DHTMLX Booking.
---
...
Следующий фрагмент кода объединяет три правила: общее, для дней недели (вторники и пятницы) и для среды, четверга и одной конкретной даты:
...
Используйте свойство [`infoTemplate`](api/config/booking-infotemplate.md), чтобы полностью заменить информационный блок по умолчанию пользовательским HTML. Если применить оба свойства, `infoTemplate` и `infoShape`, то `infoTemplate` переопределяет настройки `infoShape`.
